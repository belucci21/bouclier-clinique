...
describe('booking API', () => {
  it('returns treatments with variants, doctors, and the payment feature state', async () => {
    ...
    store.listAppointmentTypes = vi.fn().mockResolvedValue([{
      ...
      durationMinutes: 60,
      variants: [{ id: 'variant_1', name: 'Valoración', priceMxnMinor: 100000, active: true }],
    }])
    ...
    expect(result.body.appointmentTypes[0].variants).toEqual([{ id: 'variant_1', name: 'Valoración', priceMxnMinor: 100000, active: true }])
  ...
  it('subtracts blocked times, appointments, and live holds with overlap-safe calculations', async () => {
    const { service, store } = fixture()
    store.listAvailability = vi.fn().mockResolvedValue([{ dayOfWeek: 3, startTime: '10:00:00', endTime: '15:00:00' }])
    store.listBusyIntervals = vi.fn().mockResolvedValue([
  ...
  it('returns a retryable conflict when another reservation occupies the interval', async () => {
    const { service, store } = fixture()
    store.assertSlotAvailable.mockRejectedValue(Object.assign(new Error('overlap'), { code: '23505' }))
    const app = createApp({ bookingService: service, webhookHandler: vi.fn() })
